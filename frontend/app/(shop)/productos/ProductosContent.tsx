...
const cache: { productos: Producto[]; categorias: Categoria[] } = {
  productos: [],
  categorias: [],
};
...
export function ProductosContent() {
  ...
  const [productos, setProductos] = useState<Producto[]>(cache.productos);
  const [categorias, setCategorias] = useState<Categoria[]>(cache.categorias);
  const [loading, setLoading] = useState(cache.productos.length === 0);
  const [error, setError] = useState<string | null>(null);

  const [selectedCategoria, setSelectedCategoria] = useState(searchParams.get('categoria') || '');
  const [selectedTalla, setSelectedTalla] = useState(searchParams.get('talla') || '');
  ...
  const buildParams = (
    categoria: string, talla: string, min: string, max: string, search: string
  ) => {
    const p = new URLSearchParams();
    if (categoria) {
      if (isNaN(Number(categoria))) p.append('categoria_slug', categoria);
      else p.append('categoria_id', categoria);
    }
    if (talla) p.append('talla', talla);
    if (min) p.append('precio_min', min);
    if (max) p.append('precio_max', max);
  ...
  const fetchProductos = async (
    ...
    try {
      setLoading(true);
      setError(null);
      const params = buildParams(categoria, talla, min, max, search);
  ...
  // Carga inicial y al cambiar filtros via URL
  useEffect(() => {
    ...
    const key = buildParams(cat, talla, min, max, search);
    ...
    // Si el cache ya tiene datos con los mismos filtros, no fetchear
    if (cache.productos.length > 0 && key === prevParams.current) return;
    prevParams.current = key;

    fetchProductos(cat, talla, min, max, search);
  }, [searchParams.toString()]); // eslint-disable-line react-hooks/exhaustive-deps

  // Cargar categorías una sola vez
  useEffect(() => {
    if (cache.categorias.length > 0) return;
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/categorias`)
      .then(r => r.ok ? r.json() : [])
      .then((data: Categoria[]) => { cache.categorias = data; setCategorias(data); })
      .catch(() => {});
  ...
  const handleCategoriaChange = (v: string) => { setSelectedCategoria(v); updateFilters({ categoria: v }); };
  const handleTallaChange = (v: string) => { setSelectedTalla(v); updateFilters({ talla: v }); };
  ...
  const clearFilters = () => {
    setSelectedCategoria(''); setSelectedTalla('');
    setPrecioMin(''); setPrecioMax(''); setSearchQuery('');
  ...
  return (
    <div className="min-h-screen bg-amanda-white pt-16">
      ...
      <div className="lg:hidden border-b border-amanda-lightgray bg-amanda-white px-4 py-3 flex items-center gap-2">
        ...
        <select value={selectedTalla} onChange={e => handleTallaChange(e.target.value)}
          className="flex-1 min-w-0 text-[10px] tracking-widest uppercase border border-amanda-lightgray bg-transparent py-2 px-2 focus:outline-none focus:border-amanda-black text-amanda-black">
        ...
        {(selectedCategoria || selectedTalla || precioMin || precioMax) && (
          <button onClick={clearFilters} className="shrink-0 text-[10px] tracking-widest uppercase text-amanda-gray border border-amanda-lightgray px-2 py-2">✕</button>
        )}
      </div>
      ...
      <div className="max-w-screen-xl mx-auto px-6 py-8 flex flex-col lg:flex-row gap-10">
        ...
        <aside className="hidden lg:block lg:w-48 shrink-0">
          <div className="flex justify-between items-center mb-6">
            ...
            <button onClick={clearFilters} className="text-[10px] tracking-widest uppercase text-amanda-gray hover:text-amanda-black">Limpiar</button>
          </div>
          <div className="space-y-6">
            <div>
              <p className="text-[10px] tracking-widest uppercase text-amanda-gray mb-3">Categoría</p>
              <div className="space-y-2">
                <button onClick={() => handleCategoriaChange('')} className={`block text-xs tracking-wide w-full text-left ${!selectedCategoria ? 'text-amanda-black font-medium' : 'text-amanda-gray hover:text-amanda-black'}`}>Todas</button>
                {categorias.map(cat => (
                  <button key={cat.id} onClick={() => handleCategoriaChange(cat.slug)}
                    className={`block text-xs tracking-wide w-full text-left ${selectedCategoria === cat.slug ? 'text-amanda-black font-medium' : 'text-amanda-gray hover:text-amanda-black'}`}>
                    {cat.nombre}
                  </button>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {tallasDisponibles.map(t => (
                  <button key={t} onClick={() => handleTallaChange(selectedTalla === t ? '' : t)}
                    className={`text-[10px] px-2 py-1 border transition-colors ${selectedTalla === t ? 'border-amanda-black bg-amanda-black text-white' : 'border-amanda-lightgray text-amanda-gray hover:border-amanda-black hover:text-amanda-black'}`}>
                    {t}
                  </button>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                <input type="number" placeholder="Mínimo" value={precioMin} onChange={(e) => handlePrecioChange(e.target.value, precioMax)}
                  className="w-full border-b border-amanda-lightgray bg-transparent text-xs py-1 focus:outline-none focus:border-amanda-black placeholder:text-amanda-gray" />
                <input type="number" placeholder="Máximo" value={precioMax} onChange={(e) => handlePrecioChange(precioMin, e.target.value)}
                  className="w-full border-b border-amanda-lightgray bg-transparent text-xs py-1 focus:outline-none focus:border-amanda-black placeholder:text-amanda-gray" />
              </div>
            </div>
          </div>
        ...
        <div className="flex-1">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 gap-4">
              ...
              <button onClick={clearFilters} className="text-[10px] tracking-widest uppercase border-b border-amanda-black pb-0.5">Ver más prendas</button>
            </div>
          ) : productos.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 gap-4">
